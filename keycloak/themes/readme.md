...
Copy the _chaimeleon_ directory into the _themes_ directory of the keycloak directory, by default: ``/opt/keycloak/themes/``.
...
To test locally some change to the theme you can launch a container with: 
    docker run -d --name keycloak -p 10001:8080 -e KEYCLOAK_ADMIN=admin -e KEYCLOAK_ADMIN_PASSWORD=secret ^
    --mount type=bind,source=C:\proyectos\k8s\CHAIMELEON\k8s-deployments\keycloak\themes\chaimeleon,target=/opt/keycloak/themes/chaimeleon ^
